...
def MedidorId(file_name):
    ...
    medidor_id = pd.read_sql(
        'SELECT id FROM medidores_medidor WHERE serial = "{}"'.format(
            parsed_file_name),
        con=engine)
...
def ArchivoProcesado(file_name, estado):
    ...
    estado_cargue.to_sql(
        name='izarnet_izarnetprocesados',
...
def RegistrosUnicos(data):
    ...
    try:
        ...
        data = data.drop_duplicates(['fecha'], keep=False)
...
def Normalize(data):
    ...
    data['volumen_litros'] = data[headers[1]].apply(
        FloatNormalize)  # volumen en litros
    data[headers[1]] = data['volumen_litros'] / 1000  # volumen a cm
    data[headers[2]] = data[headers[2]].apply(
        FloatNormalize)  # consumo en litros
    data[headers[4]] = data[headers[4]].apply(AlarmaNormalize)  # alarmas

    # normalizando nombre de columnas
    data = data[[headers[0], headers[1], headers[
        2], 'volumen_litros', headers[4]]]
...
for file in file_names:
    ...
    if not os.path.exists(path + 'Procesados/' + now + '/'):
        os.makedirs(path + 'Procesados/' + now + '/')
    shutil.move(file, path + 'Procesados/' + now + '/')
